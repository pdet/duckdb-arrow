...
* `compression_level`: The compression level for the selected codec. For `zstd` this ranges from -131072 to 22 (default: 1, as in Arrow C++, negative levels favour speed), for `lz4` from -65536 to 12 (default: 0, i.e. the fast mode; 3 and above use LZ4HC, negative levels select an acceleration). Requires `compression`.
