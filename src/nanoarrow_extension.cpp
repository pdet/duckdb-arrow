...
inline void NanoarrowOpenSSLVersionScalarFun(DataChunk &args, ExpressionState &state, Vector &result) {
    auto &name_vector = args.data[0];
    UnaryExecutor::Execute<string_t, string_t>(
	    name_vector, result, args.size(),
	    [&](string_t name) {
...
static void LoadInternal(DatabaseInstance &instance) {
    ...
    auto nanoarrow_scalar_function = ScalarFunction("nanoarrow", {LogicalType::VARCHAR}, LogicalType::VARCHAR, NanoarrowScalarFun);
    ExtensionUtil::RegisterFunction(instance, nanoarrow_scalar_function);

    // Register another scalar function
    auto nanoarrow_openssl_version_scalar_function = ScalarFunction("nanoarrow_openssl_version", {LogicalType::VARCHAR},
...
void NanoarrowExtension::Load(DuckDB &db) {
	LoadInternal(*db.instance);
}
std::string NanoarrowExtension::Name() {
	return "nanoarrow";
}
